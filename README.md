...
<h2>Usage</h2>
To use LyricToGoogleImage, you will need to obtain an API key from Google to use their search engine. You will need to go to https://developers.google.com/custom-search/v1/introduction to obtain the key. Do not worry about anything else listed on the article, you only need to click the blue button that says "Get a Key". Follow the prompt to generate a key and hold on to that key! You will need it later.